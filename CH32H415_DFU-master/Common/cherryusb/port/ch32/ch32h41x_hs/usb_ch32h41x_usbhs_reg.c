...
#define USB_SET_RX_DMA(ep_idx, addr) (*(volatile uint32_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP1_RX_DMA) + 4 * (ep_idx - 1)) = addr)
#define USB_SET_TX_DMA(ep_idx, addr) (*(volatile uint32_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP1_TX_DMA) + 4 * (ep_idx - 1)) = addr)

#define USB_SET_MAX_LEN(ep_idx, len) (*(volatile uint16_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP0_MAX_LEN) + 4 * ep_idx) = len)
#define USB_SET_TX_LEN(ep_idx, len)  (*(volatile uint16_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP0_TX_LEN) + 4 * ep_idx) = len)
#define USB_GET_TX_LEN(ep_idx)       (*(volatile uint16_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP0_TX_LEN) + 4 * ep_idx))
#define USB_SET_TX_CTRL(ep_idx, val) (*(volatile uint8_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP0_TX_CTRL) + 4 * ep_idx) = val)
#define USB_GET_TX_CTRL(ep_idx)      (*(volatile uint8_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP0_TX_CTRL) + 4 * ep_idx))
#define USB_SET_RX_CTRL(ep_idx, val) (*(volatile uint8_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP0_RX_CTRL) + 4 * ep_idx) = val)
#define USB_GET_RX_CTRL(ep_idx)      (*(volatile uint8_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP0_RX_CTRL) + 4 * ep_idx))
...
#define EPn_GET_RX_LEN(ep_idx)       (*(volatile uint16_t *)((uint32_t)(&CH32H41x_USBHS_DEV->USB_EP0_RX_LEN) + 4 * ep_idx))
#define EPn_SET_TX_LEN(ep_idx, len)  (*(volatile uint16_t *)((uint32_t)(&CH32H41x_USBHS_DEV->UEP0_TX_LEN) + 4 * ep_idx) = len)
...
/* ch32h41x_hs usb */
static struct _ch32h41x_hs_core_prvi {
...
#ifdef CONFIG_USB_HS
    CH32H41x_USBHS_DEV->BASE_MODE = USBHS_UD_SPEED_HIGH;
#else
    CH32H41x_USBHS_DEV->BASE_MODE = USBHS_UD_SPEED_FULL;
#endif
    CH32H41x_USBHS_DEV->CONTROL = USBHS_UD_DEV_EN | USBHS_UD_DMA_EN | USBHS_UD_LPM_EN | USBHS_UD_PHY_SUSPENDM;

    CH32H41x_USBHS_DEV->UEP_T_TOG_AUTO = 0xfe;
    CH32H41x_USBHS_DEV->UEP_R_TOG_AUTO = 0xfe;
...
{
    CH32H41x_USBHS_DEV->CONTROL |= USBHS_UD_RST_SIE;
    CH32H41x_USBHS_DEV->CONTROL &= ~USBHS_UD_RST_SIE;
...
int usbd_ep_set_stall(uint8_t busid, const uint8_t ep)
{
    ...
    if (USB_EP_DIR_IS_OUT(ep)) {
        if (ep_idx == 0) {
            CH32H41x_USBHS_DEV->UEP0_RX_CTRL = USBHS_UEP_R_RES_STALL;
        } else {
            USB_SET_RX_CTRL(ep_idx, (USB_GET_RX_CTRL(ep_idx) & ~USBHS_UEP_R_RES_MASK) | USBHS_UEP_R_RES_STALL);
        }
    } else {
        if (ep_idx == 0) {
            CH32H41x_USBHS_DEV->UEP0_TX_CTRL = USBHS_UEP_T_RES_STALL;
        } else {
...
int usbd_ep_start_write(uint8_t busid, const uint8_t ep, const uint8_t *data, uint32_t data_len)
{
    ...
    if (ep_idx == 0) {
        if (data_len == 0) {
            ...
            CH32H41x_USBHS_DEV->UEP0_DMA = (uint32_t)data;
        }
    } else {
        if (data_len == 0) {
            USB_SET_TX_LEN(ep_idx, 0);
...
static inline void handle_ep0_in(void)
{
    switch (usb_dc_cfg.setup.bmRequestType >> USB_REQUEST_DIR_SHIFT) {
        case 1:
            CH32H41x_USBHS_DEV->UEP0_TX_CTRL ^= USBHS_UEP_T_TOG_DATA1;
            EPn_SET_TX_NAK(0);
        ...
        case 0:
            ...
            switch (usb_dc_cfg.setup.bRequest) {
                case USB_REQUEST_SET_ADDRESS:
                    ...
                    CH32H41x_USBHS_DEV->DEV_AD = usb_dc_cfg.address;
...
static inline void usb_process_ep_out(uint8_t epid)
{
    EPn_SET_RX_NAK(epid);
    if (epid == 0) {
        // printf("%02x\r\n",usb_dc_cfg.setup.bmRequestType);
        Delay_Us(20);
        if (CH32H41x_USBHS_DEV->UEP0_RX_CTRL & USBHS_UEP_R_SETUP_IS) {
            CH32H41x_USBHS_DEV->UEP0_RX_CTRL |= USBHS_UEP_R_TOG_DATA1;
            CH32H41x_USBHS_DEV->UEP0_TX_CTRL |= USBHS_UEP_T_TOG_DATA1;
...
{
    uint8_t epid = (CH32H41x_USBHS_DEV->INT_ST & USBHS_UDIS_EP_ID_MASK);
    switch (CH32H41x_USBHS_DEV->INT_ST & USBHS_UDIS_EP_DIR) {
