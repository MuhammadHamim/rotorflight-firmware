...
{
    const bool isBusServo = index >= BUS_SERVO_OFFSET;
    const int displayIndex = isBusServo ? index - BUS_SERVO_OFFSET + 1 : index + 1;
    if (isBusServo)
    {
        if (hasServoOverride(index))
            cliPrintLinef("servo %d (S%d): %4dus %3d OVERRIDE", index + 1, displayIndex,
                          getServoOutput(index), getServoOverride(index));
        else
            cliPrintLinef("servo %d (S%d): %4dus %3d", index + 1, displayIndex,
                          getServoOutput(index), lrintf(mixerGetServoOutput(index) * 1000));
    }
    else if (hasServoOverride(index))
    {
...
static void printServoOverride(uint8_t index)
{
    ...
    {
        if (hasServoOverride(index))
            cliPrintLinef("servo override %d (S%d) %d", index + 1, displayIndex, getServoOverride(index));
        else
            cliPrintLinef("servo override %d (S%d) off", index + 1, displayIndex);
    }
    else if (hasServoOverride(index))
    {
...
static void cliSdInfo(const char *cmdName, char *cmdline)
{
    ...
    case AFATFS_FILESYSTEM_STATE_FATAL:
        ...
        case AFATFS_ERROR_NONE:; // Nothing more detailed to print
            break;
        }
        break;
    }
...
static void cliFlashInfo(const char *cmdName, char *cmdline)
{
    ...
    // Raw JEDEC ID read at boot (0xFFFFFF = no response, 0 = not probed).
    cliPrintLinef("Flash JEDEC ID = 0x%06X", flashGetJedecId());
...
#undef BLOCK_SIZE
    ...
    if (len > 0 && signatureIsSet() && memcmp(signature, getSignature(), SIGNATURE_LENGTH))
    {
        ...
        incompatibleConfigError(cmdName, "signature", signatureStr, cmdline);
    }
    else
    {
...
{
    headingStr = cliPrintSectionHeading(dumpMask, false, headingStr);
    for (uint32_t i = 0; featureNames[i]; i++)
    { // disabled features first
        if (strcmp(featureNames[i], emptyString) != 0)
        { // Skip unused
